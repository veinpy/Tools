...
if __name__ == "__main__":
    w2v = gensim.models.Word2Vec.load('train_500epoch_09_16')
    vectors = w2v.syn0
    X = init_board_gauss(200,3)
    ks, logWks, logWkbs, sk = gap_statistic(X,ks=[2,3,5])
    print 'finished gap_statistic'
    for i in range(len(ks)-1):
        if logWkbs[i]-logWks[i]>=(logWkbs[i+1]-logWks[i+1])-sk[i+1]:
            print ks[i]
    gpp = []
    for i in range(len(ks)):
        gpp.append(logWkbs[i]-logWks[i])
